...
export function requestHead(host, ip, secure, timeoutMs = 6000) {
  return new Promise(resolve => {
    ...
    request.once('error', error => resolve({ ok: false, error: error.code === 'CERT_HAS_EXPIRED' ? 'certificado vencido' : 'sin respuesta verificable' }));
    request.end();
  });
}
...
export async function probeHost(input, { resolve = resolvePublicIPv4, request = requestHead } = {}) {
  ...
  return {
    ...
    http: { responded: plain.ok, status: plain.status ?? null, redirectsToHttps: plain.ok && /^https:\/\//i.test(plain.location ?? '') },
    https: { responded: secure.ok, status: secure.status ?? null },
